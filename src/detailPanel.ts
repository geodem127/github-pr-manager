...
export class PrDetailPanel {
    ...
    switch (msg.command) {
      ...
      case 'resolveThread':
        if (msg.threadId && this.pr) {
          await this.tryAction(
            () => this.client.resolveThread(msg.threadId!),
      ...
      case 'reRequest':
        if (msg.login && this.pr) {
          await this.tryAction(
            () => this.client.reRequestReview(this.pr!.number, msg.login!),
            `Re-requested review from @${msg.login}`,
      ...
      case 'addComment':
        if (msg.text?.trim() && this.pr) {
          await this.tryAction(
            () => this.client.addIssueComment(this.pr!.number, msg.text!.trim()),
            'Comment posted',
  ...
  private async tryAction(
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `${errPrefix}: ${err instanceof Error ? err.message : err}`
      );
    }
  }
    ...
    try {
      ...
      if (lineIdx < 0 || lineIdx >= doc.lineCount) {
        vscode.window.showWarningMessage(
          `Line ${s.line} no longer exists in ${s.path}. Apply manually.`
        );
        return;
      }
      ...
      if (s.old !== undefined && target.text !== s.old) {
        const pick = await vscode.window.showWarningMessage(
          `Line ${s.line} of ${s.path} has changed since the suggestion was made. Apply anyway?`,
          'Apply anyway',
          'Cancel'
        );
      ...
      vscode.window.showErrorMessage(
        `Failed to apply suggestion: ${err instanceof Error ? err.message : err}`
      );
    }
  }
  ...
  private commentBody(c: Comment, thread?: Thread): string {
    ...
    let html = renderMarkdown(md).replace(
      /\b([0-9a-f]{40})\b/g,
      (_m, sha: string) => `<span class="commit-ref">${sha.slice(0, 7)}</span>`
    );
  ...
  private suggestionHtml(sid: string): string {
    ...
    const oldRow =
      s.old !== undefined
        ? `<tr class="sugg-del"><td class="sign">−</td><td><code>${escapeHtml(s.old)}</code></td></tr>`
        : '';
    const newRows = s.suggestion
      .split('\n')
      .map(
        (l) => `<tr class="sugg-add"><td class="sign">+</td><td><code>${escapeHtml(l)}</code></td></tr>`
      )
      .join('');
    return `
    <div class="suggestion">
      <div class="suggestion-header">
        ...
        ${
          applicable
            ? `<button class="btn-success btn-small" data-action="applySuggestion" data-sid="${sid}" title="Apply this change to ${escapeHtml(s.path)}:${s.line} in your working tree">Apply suggestion</button>`
            : '<span class="muted small" title="Missing file/line context">not auto-applicable</span>'
        }
      </div>
  ...
  private numberedHunkHtml(hunk: string, maxRows = 12): string {
    ...
    return (
      ...
      shown
        .map((r) =>
          r.cls === 'hunk'
            ? `<tr class="hunk-row"><td colspan="3">${escapeHtml(r.text)}</td></tr>`
            : `<tr><td class="num">${r.o}</td><td class="num">${r.n}</td><td class="code ${r.cls}">${escapeHtml(r.text)}</td></tr>`
        )
        .join('') +
      '</table>'
    );
      ...
      <div class="card-body">
        ...
        <div class="thread-footer">
          ${
            thread.isResolved
              ? ''
              : `<button class="btn-success" data-action="resolveThread" data-thread="${escapeHtml(thread.id)}" title="Mark this conversation as resolved on GitHub">Resolve conversation</button>`
          }
        </div>
      </div>
    </div>`;
  ...
  private reviewerHtml(r: ReviewerInfo): string {
    const indicator =
      r.state === 'APPROVED'
        ? `<span class="rev-ind green" title="Approved">${ICONS.check}</span>`
        : r.state === 'CHANGES_REQUESTED'
          ? `<span class="rev-ind red" title="Requested changes">${ICONS.fileDiff}</span>`
          : r.state === 'COMMENTED'
            ? `<span class="rev-ind grey" title="Commented">${ICONS.comment}</span>`
            : `<span class="rev-ind yellow" title="Awaiting review">${ICONS.dot}</span>`;
    const reRequest =
      r.state === 'PENDING'
        ? ''
        : `<button class="icon-btn" data-action="reRequest" data-login="${escapeHtml(r.login)}" title="Re-request review from @${escapeHtml(r.login)}">${ICONS.sync}</button>`;
    return `
    <div class="side-row">
      ${this.avatar(r, 20)}
  ...
  private sidebarHtml(pr: PullRequest, detail: PrDetail): string {
    const reviewers = detail.reviewers.length
      ? detail.reviewers.map((r) => this.reviewerHtml(r)).join('')
      : '<div class="muted small">No reviews</div>';
    const assignees = pr.assignees.length
      ? pr.assignees
          .map(
            (a) =>
              `<div class="side-row">${this.avatar(a, 20)}<span class="side-name">${escapeHtml(a.login)}</span></div>`
          )
          .join('')
    ...
    const development = detail.linkedIssues.length
      ? detail.linkedIssues
          .map(
            (i) =>
              `<div class="side-row"><span class="rev-ind green">${ICONS.issue}</span><a href="${escapeHtml(i.url)}" class="side-name">#${i.number} ${escapeHtml(i.title)}</a></div>`
          )
          .join('')
      : '<div class="muted small">No linked issues</div>';
  ...
  private mergeBoxHtml(pr: PullRequest, detail: PrDetail): string {
    ...
    const review = detail.changesRequested
      ...
      : approved
        ? row('green', ICONS.check, 'Changes approved', 'Approving reviews are in place.')
        : row('grey', ICONS.dot, 'Review required', 'No approving reviews yet.');
    ...
    return files
      .map((f) => {
        const fileThreads = threads.filter(
          (t) => t.kind === 'review' && t.path === f.filename && t.line != null
        );
        const letter =
          f.status === 'added'
            ? '<span class="fstat fadd">A</span>'
            : f.status === 'removed'
              ? '<span class="fstat fdel">D</span>'
              : f.status === 'renamed'
                ? '<span class="fstat fren">R</span>'
                : '<span class="fstat fmod">M</span>';
        const body = f.patch
          ? this.splitDiffHtml(f.patch, fileThreads)
          : '<div class="muted small" style="padding:8px 12px">No diff available (binary or too large).</div>';
        return `
  ...
  private splitDiffHtml(patch: string, threads: Thread[]): string {
    ...
    const cell = (c: DiffCell): string =>
      `<td class="num">${c.num ?? ''}</td><td class="code ${c.type}">${c.text ? escapeHtml(c.text) : c.type === 'empty' ? '' : ' '}</td>`;
    ...
    for (const line of patch.split('\n')) {
      if (line.startsWith('@@')) {
        ...
        out.push(
          `<tr>${cell({ num: oldN++, text, type: 'ctx' })}${cell({ num: newN, text, type: 'ctx' })}</tr>`
        );
        emitComments(newN);
        newN++;
      }
